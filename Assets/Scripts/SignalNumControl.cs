...
public class SignalNumControl : MonoBehaviour
{
    ...
    public void OnButtonClick(int buttonNumber)
    {
        ...
        if(OrderController.instructionList.Count >= thisLinenum){
            OrderController.instructionList[thisLinenum - 1] = buttonNumber.ToString();
        }else{
            OrderController.instructionList.Add(buttonNumber.ToString());
        }
    }
}
