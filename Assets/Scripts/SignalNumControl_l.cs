...
public class SignalNumControl_l : MonoBehaviour
{
    public int currentCirTimes_l = 0; // 当前数字
    public TMP_Text numberText_l;
    private int thisLinenum;

    private void Start()
    {
        Transform panel = transform.Find("Sig_NumPanel");
    ...
    {
        // 更新当前数字
        currentCirTimes_l = buttonNumber;
        numberText_l.text = buttonNumber.ToString();
        OrderController_lit orderController_l = FindObjectOfType<OrderController_lit>();
        if(OrderController_lit.instructionList_l.Count >= thisLinenum){
            OrderController_lit.instructionList_l[thisLinenum - 1] = buttonNumber.ToString();
        }else{
            OrderController_lit.instructionList_l.Add(buttonNumber.ToString());
        }
    }
}
